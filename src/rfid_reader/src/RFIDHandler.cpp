...
CTagReportData *              pTagReportData)
{
	...
	*/
	...
	char * target_epc = TARGET_TAG_EPC;
	if (strcmp(epcBuf, target_epc) != 0) {
		...
		if (antenna == 1)
		{
			...
			if(DataVec1.begin() == DataVec1.end())
			{
				DataVec1.push_back(oneTagData);
			}
			else
			{
				oneTagData.phase = unwrappingPhase(oneTagData.phase, DataVec1.back().phase);
				DataVec1.push_back(oneTagData);
			}

			std_msgs::Float32 Unwrapped_phase;
			Unwrapped_phase.data = (DataVec1.back().phase-DataVec1[0].phase)*PI/180+caculate_initial_phase(antenna_xyz_data1,label_xyz_data);
			DataVec_pub1.publish(Unwrapped_phase);
		...
		if (antenna == 2)
		{
			...
			if(DataVec2.begin() == DataVec2.end())
			{
				DataVec2.push_back(oneTagData);
			}
			else
			{
				oneTagData.phase = unwrappingPhase(oneTagData.phase, DataVec2.back().phase);
				DataVec2.push_back(oneTagData);
			}

			std_msgs::Float32 Unwrapped_phase;
			Unwrapped_phase.data = (DataVec2.back().phase-DataVec2[0].phase)*PI/180+caculate_initial_phase(antenna_xyz_data2,label_xyz_data);
			DataVec_pub2.publish(Unwrapped_phase);
		...
		if (antenna == 3)
		{
			...
			if(DataVec3.begin() == DataVec3.end())
			{

				DataVec3.push_back(oneTagData);
			}
			else
			{
				oneTagData.phase = unwrappingPhase(oneTagData.phase, DataVec3.back().phase);
				DataVec3.push_back(oneTagData);
			}

			std_msgs::Float32 Unwrapped_phase;
			Unwrapped_phase.data = (DataVec3.back().phase-DataVec3[0].phase)*PI/180+caculate_initial_phase(antenna_xyz_data3,label_xyz_data);
			DataVec_pub3.publish(Unwrapped_phase);
		...
		if (antenna == 4)
		{
			...
			if(DataVec4.begin() == DataVec4.end())
			{

				DataVec4.push_back(oneTagData);
			}
			else
			{
				oneTagData.phase = unwrappingPhase(oneTagData.phase, DataVec4.back().phase);
				DataVec4.push_back(oneTagData);
			}

			std_msgs::Float32 Unwrapped_phase;
			Unwrapped_phase.data = (DataVec4.back().phase-DataVec4[0].phase)*PI/180+caculate_initial_phase(antenna_xyz_data4,label_xyz_data);
			DataVec_pub4.publish(Unwrapped_phase);
